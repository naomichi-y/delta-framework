...
/**
 * {@link Delta_DatabaseTransactionListener データベーストランザクション} を制御するリスナです。
 * リスナをアプリケーションに適用することで、アプリケーションロジックでのコミット制御 (BEGIN、COMMIT、ROLLBACK) が不要となり、プログラムが正常終了する時点で自動コミットが行われるようになります。
 * プログラム中に例外 (またはエラー) が発生した場合は、全てのトランザクションがロールバックされます。
 *
