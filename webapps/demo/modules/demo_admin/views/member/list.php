...
<html lang="ja">
  ...
  <body>
    ...
    <div id="contents">
      <p class="right"><?php echo $html->link('戻る', array('controller' => 'Index', 'action' => 'home')) ?></p>
      <?php if ($pager->hasRecord()): ?>
        ...
        <table>
          ...
          <?php while ($current = $pager->next()): ?>
            <tr>
              <td><?php echo $html->memberAvatarImage($current['member_id']) ?></td>
              <td><?php echo $html->link($current['nickname'], array('action' => 'profile'), array(), array('query' => array('memberId' => $current['member_id'], 'page' => $request->get('page')))) ?></td>
